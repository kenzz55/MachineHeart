...
#include "Behaviors/MHVeteranGuardAtk13.h"
#include "Kismet/GameplayStatics.h"
#include "Character/MHCharacterBase.h"
#include "Character/MHCharacterEnemy.h"
#include "Character/MHCharacterPlayer.h"

void UMHVeteranGuardAtk13::ExecuteBehavior_Implementation(AMHCharacterEnemy* Enemy, AMHBattleGameMode* GM)
{
    if (!Enemy) return;
    ...
    if (auto* Player = Cast<AMHCharacterPlayer>(UGameplayStatics::GetPlayerCharacter(Enemy, 0)))
    {
        if (FMath::FRand() <= 0.75f)
        {
            ...
            Player->ReceiveBattleDamageFromEnemy(Enemy, /*bUseBonus=*/true, /*Bonus=*/13.f);
            return;
        }
        else
        {
